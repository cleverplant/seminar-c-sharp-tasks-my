...
Ну а теперь я расскажу о файле [GetRequest.cs](), своими словами. [Вот принцип работы]() _так можно представить, что делает Request - отправка запроса и Response - получение ответа на запрос.
...
И результат в консоли будет похож на файл [MyJson.json](), потому что он все еще в JSON формате.

Можно на каждом этапе выполнять команду dot run и проверять ответ в программе fiddler.

_Основное мы реализовали!_

Остается вытащить интересующие нас значения погоды из JSON и привести к потному для человека виду. Как это сделать вы узнаете из коментов в самом файле [GetRequest.cs]()
