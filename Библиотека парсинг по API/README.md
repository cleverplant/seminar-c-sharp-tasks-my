...
[Так мы получаем сведения от сервера погоды картинка: ](https://github.com/cleverplant/seminar-c-sharp-tasks-my/blob/main/%D0%91%D0%B8%D0%B1%D0%BB%D0%B8%D0%BE%D1%82%D0%B5%D0%BA%D0%B0%20%D0%BF%D0%B0%D1%80%D1%81%D0%B8%D0%BD%D0%B3%20%D0%BF%D0%BE%20API/response%20%26%20reques.png)
...
_Начнем пожалуй. Все шаги записаны все патроны подсчитаны._
...
Итак. Мы будем парсить сайт прогноза погоды https://openweathermap.org/. Парсинг эмулирует действия браузера с помощью кода своей программы. Мы, как и браузер, будем отправлять Get / (Post) запросы, но из кода своей программы.
...
--------------------------------------------------------------------------
2). Создадим паблик метод Run. Этот метод в нужный момент запускает созданный запрос:
--------------------------------------------------------------------------
...
3). создаем запрос:  _request = WebRequest.Create();  
-------------------------------------------------------------------------------------------

и передаем ему адрес  _address:  _request = WebRequest.Create(_address);

Теперь приводим все к WEBREQUEST = (HttpWebRequest)WebRequest.Create(_address); Так создали готовый запрос.
...
И в этот метод будем записывать ответ от вебсервера.
...
6). Здесь создаем запрос к веб порталу:
----------------------------------------------------------------------------------------------
...
7). Чтобы выполнить данный запрос, нужно у объекта request выполнить метод RUN: 
-----------------------------------------------------------------------------------------------
...
8). нам нужно обработать полученный ответ. Для этого создаем переменную в которую пишется парсинг: var json
-----------------------------------------------------------------------------------------------
...
Так у нас будет объект, который парсит response - JObject.Parse(response)

Нам нужен пакет Newtonsoft.Json. Мы его установили и можем представить строку JSON в вид JSON формат.
...
Остается вытащить интересующие нас значения погоды из JSON и привести к понятному для человека виду. Как это сделать вы узнаете из коментов в самом файле :

 [GetRequest.cs](https://github.com/cleverplant/seminar-c-sharp-tasks-my/blob/main/%D0%91%D0%B8%D0%B1%D0%BB%D0%B8%D0%BE%D1%82%D0%B5%D0%BA%D0%B0%20%D0%BF%D0%B0%D1%80%D1%81%D0%B8%D0%BD%D0%B3%20%D0%BF%D0%BE%20API/parsing%20weather%20C%23/GetRequest.cs)
...
Этот простой шаблон можно апгрейдить и сделать следующий по: 

/*    
Т.З. 

      1. разбиваем API на части, изучаем библиотеку сервера погоды 

      2. даем возможность пользователю ввести значения ГЕО т.е. свой город 

      3. изучаем бибку и переводим на русский язык

      4. делаем защиту от дурака, привести вводимую строку к формату понятному сервисом openweathermap 
        (отпала необходимость. сервис принимает в любом регистре)

*/

Что собственно реализовано в 

[parsing weather upgreid C#](https://github.com/cleverplant/seminar-c-sharp-tasks-my/tree/main/%D0%91%D0%B8%D0%B1%D0%BB%D0%B8%D0%BE%D1%82%D0%B5%D0%BA%D0%B0%20%D0%BF%D0%B0%D1%80%D1%81%D0%B8%D0%BD%D0%B3%20%D0%BF%D0%BE%20API/parsing%20weather%20upgreid%20C%23)
